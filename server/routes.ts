...
Return JSON:
{
  ...
  // NHL games for a date (public)
  app.get('/api/nhl/games', async (req, res) => {
    try {
      const { fetchNHLGames, fetchNHLOdds, getNHLTeamName } = await import('./sports/nhl/api-client');
      const { getNHLTeamLogo } = await import('./sports/nhl/teams');
      const date = (req.query.date as string) || new Date().toISOString().split('T')[0];
      ...
      const formatted = games.map((g: any) => {
        ...
        return {
          ...
          away: g.away,
          home: g.home,
          awayName: getNHLTeamName(g.away),
          homeName: getNHLTeamName(g.home),
          awayLogo: getNHLTeamLogo(g.away),
          homeLogo: getNHLTeamLogo(g.home),
          gameTime: g.gameTime,
