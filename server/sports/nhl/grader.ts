/**
 * NHL Bet Grader — implements BetGrader for hockey.
 * Handles moneyline, puck line (spread), and totals.
 *
 * NHL-specific rules:
 * - Moneyline includes OT/SO results (full game)
 * - Puck line uses regulation + OT + SO score
 * - Totals use regulation + OT + SO score
 */

import type { BetGrader, BetSelection, GameScore, GradingResult } from '../types';

export class NHLBetGrader implements BetGrader {
  sport = 'nhl' as const;
  ...
  private gradeSpread(sel: BetSelection, awayScore: number, homeScore: number, stake: number, score: GameScore): GradingResult {
    if (!sel.team || sel.line === undefined) return { outcome: 'pending', payout: 0, notes: 'Missing team/line for puck line' };
    ...
    if (adjusted > oppScore) {
      const profit = sel.odds > 0 ? stake * (sel.odds / 100) : stake * (100 / Math.abs(sel.odds));
      return { outcome: 'win', payout: stake + profit, notes: `Puck line win (${teamScore}+${sel.line}=${adjusted} vs ${oppScore})` };
    }
    if (adjusted === oppScore) return { outcome: 'push', payout: stake, notes: 'Puck line push' };
    return { outcome: 'loss', payout: 0, notes: `Puck line loss` };
  }
  ...
  private gradeTotal(sel: BetSelection, awayScore: number, homeScore: number, stake: number): GradingResult {
    ...
    if (sel.direction === 'over') {
      if (totalGoals > sel.line) return { outcome: 'win', payout: stake + profit, notes: `Over win (${totalGoals} > ${sel.line})` };
      if (totalGoals === sel.line) return { outcome: 'push', payout: stake, notes: 'Total push' };
      return { outcome: 'loss', payout: 0, notes: `Over loss (${totalGoals} < ${sel.line})` };
    } else {
      if (totalGoals < sel.line) return { outcome: 'win', payout: stake + profit, notes: `Under win (${totalGoals} < ${sel.line})` };
      if (totalGoals === sel.line) return { outcome: 'push', payout: stake, notes: 'Total push' };
      return { outcome: 'loss', payout: 0, notes: `Under loss (${totalGoals} > ${sel.line})` };
    }
  }
  ...
  parseSelection(rawSelection: string, betType: string, game: GameScore): BetSelection {
    ...
    if (bt === 'spread' || bt === 'puckline' || bt === 'puck line') {
